...
Greedy constructive heuristic (adiciona_lista) builds an initial feasible solution.
Tabu-search local search (iteracao, using a tabu list to avoid cycling) iteratively improves the solution until all species-coverage requirements are satisfied.
Lower bound analysis is used to assess how close the heuristic solution is to optimal.
...
Variable and comment names are in Portuguese (povoamento = settlement/patch, especies = species, vizinhanca = neighbourhood); this README describes the logic in English for accessibility.
